---
layout: post
title: 2025-05-26-第015章-ECMAScript Promise
...
Promise是异步编程的一种解决方案，Promise对象代表了未来将要发生的事件，用来传递异步操作的消息。
...
除了异步操作的结果，任何其他操作都无法改变这个状态。状态变化流程只有从pending变为fulfilled和从pending变为rejected状态，只要处于fulfilled和rejected，状态就不会再变了即resolved（已定型）。
...
使用new来调用Promise的构造器来进行实例化，Promise构造函数包含一个参数和一个带有resolve（解析）和reject（拒绝）两个参数的回调。在回调中执行一些操作（例如异步），如果一切都正常，则调用 resolve，否则调用 reject。
...
Promise.all函数用于将多个Promise实例，包装成一个新的Promise实例。
...
上面代码中，Promise.all函数接受一个数组作为参数，p1、p2、p3都是Promise对象的实例。Promise.all函数的参数不一定是数组，但是必须具有 iterator接口，且返回的每个成员都是Promise实例。
...
allSettled函数返回的Promise会等待所有输入的Promise完成，不管其中是否有Promise被拒绝。
...
Promise.race函数同样是将多个Promise实例，包装成一个新的Promise实例。
...
如果Promise.all函数和Promise.race函数的参数，不是Promise实例，就会先调用Promise.resolve函数，将参数转为Promise实例，再进一步处理。
...
有时需要将现有对象转为Promise对象，Promise.resolve函数就起到这个作用。

如果Promise.resolve函数的参数，不是具有then函数的对象，则返回一个新的Promise对象，且它的状态为fulfilled。下面代码生成一个新的Promise对象的实例p，它的状态为fulfilled，所以回调函数会立即执行，Promise.resolve函数的参数就是回调函数的参数。
...
如果Promise.resolve函数的参数是一个Promise对象的实例，则会被原封不动地返回。
...
Promise.reject函数也会返回一个新的Promise实例，该实例的状态为rejected。Promise.reject函数的参数会被传递给实例的回调函数。下面代码生成一个Promise对象的实例，状态为rejected，回调函数会立即执行。
...
then方法返回的是一个新的Promise对象，因此可以采用链式写法。then方法接收两个函数作为参数，第一个参数是Promise执行成功时的回调，第二个参数是Promise执行失败时的回调，两个函数只会有一个被调用。
...
通过多次调用then函数可以添加多个回调函数，它们会按照插入顺序并且独立运行。
...
const p = new Promise(function(resolve,reject){
    ...
    function(value){ // 第五个then //reject:reject
...
使用Promise链式编程最好扁平化，注意总是返回或终止Promise链，不要嵌套Promise，否则容易导致回调地狱。
...
catch函数是then(null, rejection)的别名，用于指定发生错误时的回调函数。Promise对象的错误具有”冒泡“性质，会一直向后传递，直到被捕获为止。也就是说，错误总是会被下一个catch语句捕获。
